...
class ImageDataset(Dataset):
    def __init__(self, src_dir, csv, train, test):
        ...
        self.train = train
        self.test = test
        self.images = self.csv[:]['Path']   ## self.all_image_names
        ...
        self.val_ratio = len(self.csv) - self.train_ratio
        ...
        if self.train == True:
            ...
            self.image_names = list(self.images[-self.val_ratio:-437])
            self.label_names = list(self.labels[-self.val_ratio:-437])
            ...
            print('Total No. of Test images are: 237')
            self.image_names = list(self.images[-237:])
            self.label_names = list(self.labels[-237:])
